...
{/snippet}
{#snippet valueDisplay(value: string, iconName?: Icon, href?: string)}
	{#if href}
		<a
			href={`https://${href}`}
			target="_blank"
			class="font-semibold text-white flex items-center gap-0.5 hover:cursor-pointer"
		>
			{#if iconName}
	...
	{:else}
		<span class="font-semibold text-white flex items-center gap-0.5">
			{#if iconName}
				{@render icon(iconName)}
			{/if}
			{value}
...
{/snippet}
{#snippet icon(icon: Icon)}
	{#if icon === 'github'}
		<svg
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 640 640"
			class="h-5 fill-light-10 dark:fill-dark-10 inline-block"
...
>
	...
	/>
	<header class="flex flex-col gap-5 grow justify-evenly @desktop:justify-start">
		...
		<div>
			...
			{@render informationValues(
				...
				'LeonardoBevilacqua',
				'github',
				'github.com/LeonardoBevilacqua'
			)}
			{@render informationValues(
				'LinkedIn',
				'LeonardoABevilacqua',
				'linkedin',
				'linkedin.com/in/leonardoabevilacqua/'
			)}
		</div>
	</header>
</div>
